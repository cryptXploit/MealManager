    ...
    <div className={`flex flex-col h-[100dvh] w-full transition-colors duration-300 overflow-hidden ${darkMode ? 'app-dark' : 'app-light bg-white'}`}>
      ...
      <header className={`flex-none z-20 px-4 py-3 border-b flex justify-between items-center backdrop-blur-md pt-safe ${darkMode ? 'bg-slate-900/80 border-slate-700' : 'bg-white/90 border-slate-100'}`}>
        ...
        <div className="flex gap-2">
          ...
          <button onClick={() => { setConfirmModal({ message: 'Log out?', onConfirm: async () => { await supabase.auth.signOut(); localStorage.clear(); setProfile(null); window.location.reload() } }) }} className={`w-8 h-8 rounded-full flex items-center justify-center transition ${darkMode ? 'bg-slate-800 text-rose-400' : 'bg-rose-50 text-rose-500'}`}><i className="fa-solid fa-right-from-bracket"></i></button>
        </div>
      </header>
