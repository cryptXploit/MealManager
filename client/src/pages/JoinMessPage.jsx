...
const JoinMessPage = ({ profile, session, setProfile, setMessDetails, setUiState }) => {
  ...
  return (
    <div className={`min-h-screen flex flex-col items-center justify-center p-6 ${darkMode ? 'app-dark' : 'app-light'}`}>
      <div className={`w-full max-w-md p-8 rounded-2xl shadow-xl border ${darkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-100'}`}>
        <h1 className="text-xl font-bold mb-8 text-center">Join Existing Mess</h1>
        ...
        <button onClick={handleJoinMess} disabled={loading} className="w-full mt-6 bg-indigo-600 text-white py-3 rounded-lg font-bold">
          {loading ? <i className="fa-solid fa-circle-notch fa-spin"></i> : 'Join Mess'}
        </button>

        <button onClick={() => setUiState('create_mess')} className="w-full mt-4 text-sm font-bold text-indigo-500 hover:text-indigo-600 transition">
